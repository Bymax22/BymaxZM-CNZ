...
import { buildProxyInit, sanitizePayload } from '../../../../lib/staffProxy';
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Staff projects proxy GET error:', error);
  ...
  try {
    ...
    if (status >= 200 && status < 300) {
      try {
        ...
        emitUpdate({ resource: 'project', action: 'created', id: data?.id, data });
      } catch {
    ...
    console.error('Staff projects proxy POST error:', error);
