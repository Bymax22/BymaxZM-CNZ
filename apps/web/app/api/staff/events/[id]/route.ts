...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Staff event detail proxy GET error:', error);
  ...
  try {
    ...
    if (status >= 200 && status < 300) {
      try {
        ...
        emitUpdate({ resource: 'event', action: 'updated', id: data?.id, data });
      } catch {
    ...
    console.error('Staff event detail proxy PUT error:', error);
    return NextResponse.json({ error: 'Failed to update event' }, { status: 500 });
  }
}

export async function DELETE(request: NextRequest) {
  return NextResponse.json({ error: 'Staff users are not allowed to delete events' }, { status: 405 });
}
