...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Staff users proxy GET error:', error);
    return NextResponse.json({ error: 'Failed to fetch users' }, { status: 500 });
  }
}

export async function POST(request: NextRequest) {
  return NextResponse.json({ error: 'Staff users are not allowed to create users' }, { status: 405 });
}

export async function PUT(request: NextRequest) {
  return NextResponse.json({ error: 'Staff users are not allowed to update users' }, { status: 405 });
}

export async function DELETE(request: NextRequest) {
  return NextResponse.json({ error: 'Staff users are not allowed to delete users' }, { status: 405 });
}
